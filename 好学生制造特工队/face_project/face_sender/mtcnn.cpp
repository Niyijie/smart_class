...
bool camScore(Bbox lsh, Bbox rsh)
{
...
{
#if(OPENCV==1)
	std::vector<std::string> modelTxts = {
		model_path + "/det1.prototxt",
		model_path + "/det2.prototxt",
		model_path + "/det3.prototxt"
	};

	std::vector<std::string> modelbins = {
		model_path + "/det1.caffemodel",
		model_path + "/det2.caffemodel",
		model_path + "/det3.caffemodel"
	};
	Pnet = dnn::readNetFromCaffe(modelTxts[0], modelbins[0]);
	Rnet = dnn::readNetFromCaffe(modelTxts[1], modelbins[1]);
	Onet = dnn::readNetFromCaffe(modelTxts[2], modelbins[2]);
#else
...
void MTCNN::nms(vector<Bbox> &boundingBox_, const float overlap_threshold, string modelname /* = "Union" */)
{
	if (boundingBox_.empty())
	{
		return;
	}
	sort(boundingBox_.begin(), boundingBox_.end(), camScore);
	float IOU=0, maxX=0, maxY=0, minX=0, minY = 0;
	...
	vPick.resize(nPick);
	std::vector<Bbox> temp_;
	temp_.resize(nPick);
	for (int i = 0; i < nPick; i++)
	{
		temp_[i] = boundingBox_[vPick[i]];
	}
	boundingBox_ = temp_;
}

void MTCNN::refine(vector<Bbox> &vecBox, const int &height, const int &width, bool square)
{
	if (vecBox.empty())
	{
	...
	for (vector<Bbox>::iterator it = vecBox.begin(); it != vecBox.end();it++)
	{
		bbw = (*it).x2 - (*it).x1 + 1;
		bbh = (*it).y2 - (*it).y1 + 1;
...
{
	firstBbox_.clear();
	float min1 = img_w < img_h ? img_w : img_h;
	float m = (float)MIN_DET_SIZE / minsize;
	min1 *= m;
	float factor = pre_facetor;
	vector<float> scales_;
	while (min1 > MIN_DET_SIZE)
	{
		scales_.push_back(m);
		min1 *= factor;
		m *= factor;
...
void MTCNN::ONet()
{
	...
	for (vector<Bbox>::iterator it = secondBbox_.begin(); it != secondBbox_.end(); it++)
	{
		...
		ex.set_light_mode(true);
		ex.input("data", in);
		ncnn::Mat score1, bbox1;
		ex.extract("prob1", score1);
		ex.extract("conv6-2", bbox1);
		if ((float)score1[1] > threshold[2])
		{
			for (int channel = 0; channel < 4; channel++)
			{
				it->regreCoord[channel] = (float)bbox1[channel];
			}
			it->area = (it->x2 - it->x1)*(it->y2 - it->y1);
			it->score = score1[1];
...
void MTCNN::NcnnDetect(ncnn::Mat &img_, std::vector<Bbox> &finalBbox_)
{
	img = img_;
	img_w = img.w;
	img_h = img.h;
