...
void MTCNN::ONet(){
    ...
    for(vector<Bbox>::iterator it=secondBbox_.begin(); it!=secondBbox_.end();it++){
        ...
        copy_cut_border(img, tempIm, (*it).y1, img_h-(*it).y2, (*it).x1, img_w-(*it).x2);
		//copy_cut_border(img, tempIm, 79,137,149,211); //79, 204, 149, 283
        ncnn::Mat in;
        resize_bilinear(tempIm, in, 48, 48);

		//cv::Mat new_img = cv::imread("in.jpg");
		//ncnn::Mat ncnn_img12 = ncnn::Mat::from_pixels(new_img.data, ncnn::Mat::PIXEL_BGR2RGB, new_img.cols, new_img.rows);
		//ncnn_img12.substract_mean_normalize(mean_vals, norm_vals);
		...
		if ((float)score[1] > threshold[2]) {
			for (int channel = 0; channel < 4; channel++) {
				...
				//cout << " x  "<<it->regreCoord[channel] << endl;
			}
			it->area = (it->x2 - it->x1) * (it->y2 - it->y1);
